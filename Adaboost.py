...
Ada_clf = Pipeline([('vect', CountVectorizer(tokenizer=LemmaTokenizer(), ngram_range=(1,2), binary=True)),
                       ('clf', AdaBoostClassifier())])
start=time.time()

print("AdaBoost Classsifier pipeline created!")
Ada_clf.fit(X_training, Y_training)

print("Model fitted!")

Y_pred_LogReg = Ada_clf.predict(X_validation)

Ada_Accuracy = np.sum(np.logical_not(np.logical_xor(Y_validation,Y_pred_LogReg)))/N_valid
print('Validation Accuracy for AdaBoost Classifier with binary features is: '+ str(Ada_Accuracy))


# 2) using TfIDF 

Ada_clf = Pipeline([('tfidf', TfidfVectorizer(tokenizer= LemmaTokenizer(), ngram_range=(1,2), sublinear_tf=True)),
                       ('norm', Normalizer()),
                       ('clf', AdaBoostClassifier())])

print("AdaBoost Classifier pipeline created!")
Ada_clf.fit(X_training, Y_training)

print("Model fitted!")

Y_pred_Ada= Ada_clf.predict(X_validation)

Ada_Accuracy = np.sum(np.logical_not(np.logical_xor(Y_validation,Y_pred_Ada)))/N_valid
print('Validation Accuracy for AdaBoost Classifier with TfIDF is: '+ str(Ada_Accuracy))
...
Ada_clf = Pipeline([('tfidf', TfidfVectorizer(sublinear_tf=True, ngram_range=(1,2),tokenizer=LemmaTokenizer())),
                    ('norm', Normalizer()),
                    ('clf', AdaBoostClassifier(learning_rate=0.05,n_estimators=50))])
...
Ada_Accuracy = np.sum(np.logical_not(np.logical_xor(Y_validation,Y_pred_Ada)))/N_valid
print('Validation Accuracy for SVM with TfIDF is: '+ str(Ada_Accuracy))
...
#grid_search_cv = GridSearchCV(Ada_clf, param_grid, verbose=1,  cv=3, n_jobs=-1)
#
